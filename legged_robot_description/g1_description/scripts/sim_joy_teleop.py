...
class SimJoyTeleop(Node):
    def __init__(self):
        ...
        # Scale config
        self.scale_lin_x = 0.3
        self.scale_lin_y = 0.3
        self.scale_ang_z = 0.2
        self.deadzone = 0.05
 
        self.lb_button_idx = 4
        self.rb_button_idx = 5  
    ...
    def joy_callback(self, msg):
        ...
        if msg.buttons[self.lb_button_idx] == 1:
            if msg.buttons[0] == 1 and self.last_buttons[0] == 0: # LB + A
                self.switch_controller(['rl_controller_whole'], ['static_controller', 'rl_controller_whole'])
            elif msg.buttons[2] == 1 and self.last_buttons[2] == 0: # LB + X
                pass
            elif msg.buttons[1] == 1 and self.last_buttons[1] == 0: # LB + B
                self.switch_controller(['static_controller'], ['rl_controller_whole', 'static_controller'])
            elif msg.buttons[3] == 1 and self.last_buttons[3] == 0: # LB + Y
                pass

        # Logic turn off publish /cmd_vel
        if msg.buttons[self.rb_button_idx] == 1:
            if msg.buttons[3] == 1 and self.last_buttons[3] == 0: # RB + Y
                self.publish_enabled = not self.publish_enabled # (Toggle)
                status = "ENABLED" if self.publish_enabled else "DISABLED"
                self.get_logger().info(f"Continuous publishing is now: {status}")
