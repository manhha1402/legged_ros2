...
controller_interface::CallbackReturn LeggedRLController::on_init() {
  ...
  try {
    param_listener_ =
        std::make_shared<legged_rl_controller::ParamListener>(get_node());
  } catch (const std::exception &e) {
    fprintf(stderr,
            "Exception thrown during LeggedRLController's on_init with "
            "message: %s \n",
            e.what());
    return controller_interface::CallbackReturn::ERROR;
  }
...
controller_interface::CallbackReturn LeggedRLController::on_configure(
  ...
  RCLCPP_INFO(get_node()->get_logger(),
              "Legged RL Controller configured successfully.");

  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn
LeggedRLController::on_activate(const rclcpp_lifecycle::State &previous_state) {
  ...
  RCLCPP_INFO(get_node()->get_logger(),
              "Legged RL Controller activated successfully.");
...
LeggedRLController::update(const rclcpp::Time & /*time*/,
                           const rclcpp::Duration &period) {

  // fall detection
  if (detect_fall_()) {
    RCLCPP_WARN(get_node()->get_logger(),
                "Robot is falling! Stopping the controller.");
  ...
  joint_interface_->set_joint_command(
      action, std::vector<float>(joint_names_.size(), 0.0f),
      std::vector<float>(joint_names_.size(), 0.0f),
      env_->robot->data.joint_stiffness, env_->robot->data.joint_damping);
...
controller_interface::CallbackReturn
LeggedRLController::configure_parameters_() {
  update_parameters_();
  ...
  for (size_t i = 0; i < joint_names_.size(); ++i) {
    ...
    if (params_.action_cfg.joint_names_map.find(joint_name) !=
        params_.action_cfg.joint_names_map.end()) {
      const auto &action_cfg =
          params_.action_cfg.joint_names_map.at(joint_name);
      // default pos
      ...
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Joint name %s not found in action configuration",
                   joint_name.c_str());
      return controller_interface::CallbackReturn::ERROR;
    }
  }
  ...
  for (const auto &obs_name : params_.obs_names) {
    ...
    if (obs_cfg_map.find(obs_name) == obs_cfg_map.end()) {
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Observation name %s not found in observation configuration",
                   obs_name.c_str());
      return controller_interface::CallbackReturn::ERROR;
    }
    ...
    if (obs_cfg.clip.size() == 2) {
      obs_term_cfg_ptr->clip = {static_cast<float>(obs_cfg.clip[0]),
                                static_cast<float>(obs_cfg.clip[1])};
    } else {
      // obs_term_cfg_ptr->clip = { -std::numeric_limits<float>::infinity(),
      // std::numeric_limits<float>::infinity() };
    ...
    std::cout << "[LeggedRLController] Observation " << obs_name
              ...
              << ", scale: " << obs_cfg.scale << ", clip: [" << obs_cfg.clip[0]
              << ", " << obs_cfg.clip[1] << "]" << std::endl;
  }
  ...
  RCLCPP_INFO(get_node()->get_logger(),
              "LeggedRLController parameters configured");
...
PLUGINLIB_EXPORT_CLASS(legged::LeggedRLController,
