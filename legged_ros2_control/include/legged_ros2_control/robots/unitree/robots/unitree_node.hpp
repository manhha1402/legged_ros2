...
template <typename UnitreeSubscriberType> class UnitreeNode {
public:
  UnitreeNode()
      : node_(std::make_shared<rclcpp::Node>(
        ...
        switch_controller_request_(
            std::make_shared<
                controller_manager_msgs::srv::SwitchController::Request>()),
        jump_cmd_pub_(node_->create_publisher<std_msgs::msg::Bool>(
            "jump_cmd", rclcpp::SystemDefaultsQoS())),
        jump_cmd_msg_(),
        jump_dist_pub_(node_->create_publisher<std_msgs::msg::Float32>(
    ...
    switch_controller_request_->strictness =
    ...
    RCLCPP_INFO(node_->get_logger(),
                "Unitree Node initialized with network interface: %s. Waiting "
                "for low state messages...",
                unitree_net_if_.c_str());
  ...
  void run() {
    ...
    while (rclcpp::ok()) {
      ...
      cmd_vel_msg_.linear.x =
          cmd_vel_scale_.lin_vel_x * low_state_subscriber_->joystick.ly();
...
private:
  void wait_for_service() const {
    while (
        !controller_switch_client_->wait_for_service(std::chrono::seconds(1))) {
      RCLCPP_WARN(
          node_->get_logger(),
  ...
  void handle_controller_switch() {
    if (low_state_subscriber_->joystick.A.on_pressed &&
      ...
      if (jump_dist_msg_.data > max_dist_)
        jump_dist_msg_.data = max_dist_;
      jump_dist_pub_->publish(jump_dist_msg_);
      RCLCPP_INFO(node_->get_logger(), "Jump distance increased to: %.2f",
                  jump_dist_msg_.data);
    } else if (low_state_subscriber_->joystick.down.on_pressed) {
      jump_dist_msg_.data -= delta_dist_;
      if (jump_dist_msg_.data < min_dist_)
        jump_dist_msg_.data = min_dist_;
      jump_dist_pub_->publish(jump_dist_msg_);
      RCLCPP_INFO(node_->get_logger(), "Jump distance decreased to: %.2f",
                  jump_dist_msg_.data);
    }
  }

  void set_controller_switch(const std::vector<std::string> &activate,
