...
#include "legged_ros2_control/robots/unitree/dds_wrapper/common/joystick.hpp"

namespace unitree {
namespace robot {

template <typename MessageType> class SubscriptionBase {
public:
  ...
  SubscriptionBase(const std::string &topic) {
    ...
    sub_ =
        std::make_shared<unitree::robot::ChannelSubscriber<MessageType>>(topic);
  ...
  void wait_for_connection() {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(100)); // wait for stable communicaiton
    if (warn_info) {
