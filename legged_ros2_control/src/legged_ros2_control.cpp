...
LeggedRos2Control::LeggedRos2Control(rclcpp::Node::SharedPtr node)
    : node_(node),
      logger_(rclcpp::get_logger(node_->get_name() +
                                 std::string(".legged_ros2_control"))) {}
...
std::string LeggedRos2Control::get_robot_description_() {
  // Getting robot description from parameter first. If not set trying from
  // topic
  ...
  RCLCPP_WARN(logger_, "Failed to get robot_description from parameter. Will "
                       "listen on the ~/robot_description "
                       "topic...");
...
void LeggedRos2Control::init() {
  ...
  try {
    hardware_info =
        hardware_interface::parse_control_resources_from_urdf(urdf_string_);
  } catch (const std::runtime_error &ex) {
    RCLCPP_ERROR_STREAM(logger_,
                        "Error parsing hardware info from URDF: " << ex.what());
    return;
  }
  ...
  try {
    resource_manager->load_urdf(
        urdf_string_, false,
        false); // Do not validate interfaces and do not load components
  } catch (const std::runtime_error &ex) {
    RCLCPP_ERROR_STREAM(
        logger_, "Error loading URDF in resource manager: " << ex.what());
    return;
  }
  ...
  controller_manager_ = std::make_shared<controller_manager::ControllerManager>(
      std::move(resource_manager), cm_executor_, manager_node_name,
      node_->get_namespace());
  cm_executor_->add_node(controller_manager_);
  cm_executor_->add_node(node_);

  const bool use_sim_time =
      controller_manager_->get_parameter_or<bool>("use_sim_time", false);
  RCLCPP_INFO(logger_, "Controller manager using simulation time: %s",
              use_sim_time ? "true" : "false");

  const int cpu_affinity =
      controller_manager_->get_parameter_or<int>("cpu_affinity", -1);
  if (cpu_affinity >= 0) {
    const auto affinity_result =
        realtime_tools::set_current_thread_affinity(cpu_affinity);
    if (!affinity_result.first) {
      RCLCPP_WARN(logger_, "Unable to set the CPU affinity : '%s'",
  ...
  if (lock_memory) {
    ...
    if (!lock_result.first) {
      RCLCPP_WARN(logger_, "Unable to lock memory : '%s'",
  ...
  const int thread_priority =
      controller_manager_->get_parameter_or<int>("thread_priority", 50);
  RCLCPP_INFO(logger_, "Spawning %s RT thread with scheduler priority: %d",
  ...
  cm_thread_ = std::thread([&]() {
    ...
    rclcpp::Duration period_error_threshold =
        rclcpp::Duration::from_nanoseconds(0.1 *
                                           period.count()); // 10% threshold
  ...
  // if (pthread_setschedparam(cm_thread_.native_handle(), SCHED_FIFO, &sched)
  // != 0) {
  //   RCLCPP_ERROR(logger_, "Failed to set thread scheduling policy to FIFO");
  // } else {
  //   RCLCPP_INFO(logger_, "Controller manager thread scheduling policy set to
  //   FIFO");
  // }
...
void LeggedRos2Control::import_components_(
  ...
  try {
    system_interface_loader_.reset(
        new pluginlib::ClassLoader<LeggedSystemInterface>(
  ...
  for (const auto &hw_info : hardware_info) {
    ...
    try {
      ...
      RCLCPP_ERROR_STREAM(logger_, "Failed to create system interface for "
                                       << hw_class_type << ": " << ex.what());
      continue;
    }
