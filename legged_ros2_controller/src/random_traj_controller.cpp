...
controller_interface::CallbackReturn RandomTrajController::on_init() {
  ...
  try {
    param_listener_ =
        std::make_shared<static_controller::ParamListener>(get_node());
  } catch (const std::exception &e) {
    fprintf(stderr,
            "Exception thrown during RandomTrajController's on_init with "
            "message: %s \n",
            e.what());
    return controller_interface::CallbackReturn::ERROR;
  }
...
controller_interface::CallbackReturn RandomTrajController::on_configure(
...
controller_interface::CallbackReturn RandomTrajController::on_activate(
    const rclcpp_lifecycle::State &previous_state) {
  ...
  for (size_t i = 0; i < joint_names_.size(); ++i) {
    joint_curves_[i].reset(init_joint_pos[i], targetPos_[targetIdx_][i],
                           current_time, current_time + params_.traj_time);
  }
...
RandomTrajController::update(const rclcpp::Time &time,
  ...
  if (current_time > joint_curves_[0].getEndTime()) {
    ...
    for (size_t i = 0; i < joint_names_.size(); ++i) {
      ...
      joint_curves_[i].reset(current_joint_pos[i],
                             targetPos_[targetIdx_][i] + noise, current_time,
  ...
  try {
    joint_interface_->set_joint_command(joint_pos_target, joint_vel_target, ff,
                                        kp_, kd_);
  } catch (const std::exception &e) {
    RCLCPP_ERROR(get_node()->get_logger(), "Error in set_joint_command: %s",
                 e.what());
...
controller_interface::CallbackReturn RandomTrajController::on_deactivate(
    const rclcpp_lifecycle::State &previous_state) {
...
PLUGINLIB_EXPORT_CLASS(legged::RandomTrajController,
