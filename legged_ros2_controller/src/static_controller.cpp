...
controller_interface::CallbackReturn StaticController::on_init() {
  ...
  try {
    ...
    fprintf(stderr,
            "Exception thrown during StaticController's on_init with message: "
            "%s \n",
            e.what());
    return controller_interface::CallbackReturn::ERROR;
  }
...
StaticController::on_configure(const rclcpp_lifecycle::State &previous_state) {
...
controller_interface::CallbackReturn
StaticController::on_activate(const rclcpp_lifecycle::State &previous_state) {
  ...
  for (size_t i = 0; i < joint_names_.size(); ++i) {
    joint_curves_[i].reset(init_joint_pos[i], default_pos_[i], current_time,
...
StaticController::update(const rclcpp::Time &time,
...
controller_interface::CallbackReturn
StaticController::on_deactivate(const rclcpp_lifecycle::State &previous_state) {
...
PLUGINLIB_EXPORT_CLASS(legged::StaticController,
