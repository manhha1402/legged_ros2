...
controller_interface::CallbackReturn LeggedController::on_configure(
  ...
  if (joint_names_.empty()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Joint names are empty, please get them in on_init() method.");
  ...
  for (const auto &imu : imu_names_) {
    imu_interfaces_.emplace_back(
        std::make_shared<semantic_components::IMUSensor>(imu));
  }
  return CallbackReturn::SUCCESS;
}

controller_interface::InterfaceConfiguration
LeggedController::command_interface_configuration() const {
...
controller_interface::InterfaceConfiguration
LeggedController::state_interface_configuration() const {
  controller_interface::InterfaceConfiguration conf;
  conf.type = config_type::INDIVIDUAL;
  ...
  for (const auto &imu_if : imu_interfaces_) {
    ...
    imu_state_if_names.insert(imu_state_if_names.end(), imu_if_names.begin(),
                              imu_if_names.end());
  }
  conf.names.reserve(joint_state_if_names.size() + imu_state_if_names.size());
  conf.names.insert(conf.names.end(), joint_state_if_names.begin(),
                    joint_state_if_names.end());
  conf.names.insert(conf.names.end(), imu_state_if_names.begin(),
                    imu_state_if_names.end());
  return conf;
}

controller_interface::CallbackReturn
LeggedController::on_activate(const rclcpp_lifecycle::State &) {
  bool success = true;
  success &=
      joint_interface_->assign_loaned_state_interfaces(state_interfaces_);
  for (auto &imu_if : imu_interfaces_) {
    success &= imu_if->assign_loaned_state_interfaces(state_interfaces_);
  }

  success &=
      joint_interface_->assign_loaned_command_interfaces(command_interfaces_);

  return success ? controller_interface::CallbackReturn::SUCCESS
                 : controller_interface::CallbackReturn::ERROR;
}

controller_interface::CallbackReturn
LeggedController::on_deactivate(const rclcpp_lifecycle::State &) {
