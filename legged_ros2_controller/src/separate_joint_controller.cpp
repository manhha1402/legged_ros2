...
controller_interface::CallbackReturn SeparateJointController::on_init() {
  ...
  if (kp_.size() != joint_names_.size() || kd_.size() != joint_names_.size()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Size of kp or kd does not match the number of joints.");
    return controller_interface::CallbackReturn::ERROR;
  }
...
SeparateJointController::update(const rclcpp::Time & /*time*/,
  ...
  try {
    joint_interface_->set_joint_command(joint_cmd_msg_->position,
                                        joint_cmd_msg_->velocity,
                                        joint_cmd_msg_->effort, kp_, kd_);
...
PLUGINLIB_EXPORT_CLASS(legged::SeparateJointController,
